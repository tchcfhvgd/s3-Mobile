...
class Paths
{
	 ...
	 */
	public static inline final CORE_DIRECTORY = #if ASSET_REDIRECT trail + 'assets' #else 'assets' #end;
	...
	public static var currentTrackedAtlasFrames:Map<String, FlxAtlasFrames> = []; // this might be a idea cuz parsing xmls constantly in cases is bad.
	...
	public static var dumpExclusions:Array<String> = ['$CORE_DIRECTORY/shared/music/freakyMenu.$SOUND_EXT'];
	...
	{
		key = getPath('images/$key.png', IMAGE, library, true);
		if (currentTrackedAssets.exists(key))
		{
			localTrackedAssets.push(key);
			return currentTrackedAssets.get(key);
		}
		return cacheBitmap(key, null, allowGPU);
	}
	
	static public function cacheBitmap(file:String, ?bitmap:BitmapData, ?allowGPU:Bool = true)
	{
		if (bitmap == null)
		{
			#if MODS_ALLOWED
			...
			if (bitmap == null)
			{
				trace('bitmap at [$file] was not found');
				return null;
			}
		}
	...
	public static function fileExists(key:String, type:AssetType, ?ignoreMods:Bool = false, ?library:String = null)
	{
		...
		final bPath = getPath(key, type, library, false);
		
		if (#if sys FileSystem.exists(bPath) || #end OpenFlAssets.exists(bPath))
		{
			return true;
		}
		return false;
	...
	public static function returnSound(path:Null<String>, key:String, ?library:String)
	{
		...
		if (!currentTrackedSounds.exists(gottenPath))
		{
			...
			if (FileSystem.exists(strip(retKey)))
			{
				currentTrackedSounds.set(gottenPath, Sound.fromFile(strip(retKey)));
			}
	...
	{
		#if hl
		var path:String = getPath('images/$folderOrImg', BINARY, null, true);
		if (path.endsWith('.png')) path.substr(0, path.lastIndexOf('/') - 1);
		spr.loadAtlas(path);
		return;
		#end
