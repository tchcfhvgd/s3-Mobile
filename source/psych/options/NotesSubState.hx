...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		copyButton = new FlxSprite(760, 50).loadGraphic(Paths.image('menu/options/copy'));
		copyButton.setGraphicSize(0, 72);
		copyButton.updateHitbox();
		copyButton.alpha = 0.6;
		add(copyButton);
		
		pasteButton = new FlxSprite(1180, 50).loadGraphic(Paths.image('menu/options/paste'));
		pasteButton.setGraphicSize(0, 72);
		...
		add(alphabetB);
		alphabetHex = makeColorAlphabet(txtX - 25, txtY - 55);
		add(alphabetHex);
		...
		tip.setFormat(Paths.font("SeuratPro.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tip.borderSize = 2;
		add(tip);
		
		tipTxt = new FlxText(tipX, tipY + 24, 0, '', 16);
		tipTxt.setFormat(Paths.font("SeuratPro.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipTxt.borderSize = 2;
		add(tipTxt);
		updateTip();
	...
	function centerHexTypeLine()
	{
		...
		if (hexTypeNum > 0)
		{
			var letter = alphabetHex.text.charAt(hexTypeNum - 1);
			// hexTypeLine.x = letter.x - letter.offset.x + letter.width;
		}
		else
		{
			var letter = alphabetHex.text.charAt(0);
	...
	{
		var text:FlxText = new FlxText(x, y, 0, '', 24);
		text.alignment = CENTER;
		text.font = Paths.font('FOT-NewRodin Pro DB.otf');
		// text.setScale(0.6);
		add(text);
		return text;
	}
