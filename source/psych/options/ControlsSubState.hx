...
import psych.options.BaseOptionsMenu.OptionsText;
...
class ControlsSubState extends MusicBeatSubstate
{
	...
	var bg:FlxSprite;
	var grpDisplay:FlxTypedGroup<OptionsText>;
	var grpBlacks:FlxTypedGroup<AttachedSprite>;
	var grpOptions:FlxTypedGroup<OptionsText>;
	var grpBinds:FlxTypedGroup<OptionsText>;
	...
	public function new()
	{
		...
		bg = new FlxSprite().makeScaledGraphic(FlxG.width, FlxG.height, FlxColor.WHITE);
		FlxTween.tween(bg, {alpha: 0.2}, 0.4);
		bg.alpha = 0;
		...
		grpDisplay = new FlxTypedGroup<OptionsText>();
		add(grpDisplay);
		...
		add(selectSpr);
		grpBinds = new FlxTypedGroup<OptionsText>();
		add(grpBinds);
		
		controllerSpr = new FlxSprite(50, 40).loadGraphic(Paths.image('menu/options/keyboard'));
		controllerSpr.setGraphicSize(0, 82);
		controllerSpr.updateHitbox();
		controllerSpr.antialiasing = ClientPrefs.data.antialiasing;
		add(controllerSpr);
		
		var text:OptionsText = new OptionsText(controllerSpr.x, controllerSpr.y + controllerSpr.height + 5, controllerSpr.width, 'CTRL', 24);
	...
	function createTexts()
	{
		curOptions = [];
		curOptionsValid = [];
		grpDisplay.forEachAlive(function(text:OptionsText) text.destroy());
		grpBlacks.forEachAlive(function(black:AttachedSprite) black.destroy());
		grpOptions.forEachAlive(function(text:OptionsText) text.destroy());
		grpBinds.forEachAlive(function(text:OptionsText) text.destroy());
		grpDisplay.clear();
		grpBlacks.clear();
		grpOptions.clear();
		grpBinds.clear();
		...
		for (i in 0...options.length)
		{
			...
			if (option[0] || onKeyboardMode)
			{
				if (option.length > 1)
				{
					...
					var text:OptionsText = new OptionsText(200, 300, 0, option[1], 48);
	...
	function addCenteredText(text:OptionsText, option:Array<Dynamic>, id:Int)
	{
		text.screenCenter(X);
		text.y -= 55;
	...
	function addKeyText(text:OptionsText, option:Array<Dynamic>, id:Int)
	{
		for (n in 0...2)
		{
			...
			if (onKeyboardMode)
			{
				var savKey:Array<Dynamic> = ClientPrefs.keyBinds.get(option[2]);
			...
			var attach:OptionsText = new OptionsText(textX + 210, 300, 0, key, 48);
	...
	{
		var bind:OptionsText = grpBinds.members[num];
		var attach:OptionsText = new OptionsText(350 + (num % 2) * 300, 300, 0, text, 48);
		// attach.isMenuItem = true;
		attach.changeX = false;
		attach.distancePerItem.y = 60;
	...
	var bindingText:OptionsText;
	var bindingText2:OptionsText;
		...
		if (!binding)
		{
			...
			if (FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A))
			{
				if (options[curOptions[curSelected]][1] != defaultKey)
				{
					...
					bindingText = new OptionsText(0, 160, FlxG.width, "Rebinding " + options[curOptions[curSelected]][3], 48);
					bindingText.alignment = CENTER;
					add(bindingText);
					
					bindingText2 = new OptionsText(0, 340, FlxG.width, "Hold ESC to Cancel\nHold Backspace to Delete", 48);
			...
			else
			{
				...
				if (changed)
				{
					...
					for (n in 0...2)
					{
						...
						if (onKeyboardMode)
						{
							var savKey:Array<Dynamic> = ClientPrefs.keyBinds.get(option);
							key = InputFormatter.getKeyName(savKey[n] != null ? savKey[n] : NONE);
						}
						else
						{
	...
	function updateText(?move:Int = 0)
	{
		...
		grpDisplay.forEachAlive(function(item:OptionsText) {
			item.targetY = item.ID - num - addNum;
		});
		
		grpOptions.forEachAlive(function(item:OptionsText) {
			item.targetY = item.ID - num - addNum;
			item.alpha = (item.ID - num == 0) ? 1 : 0.6;
		});
		grpBinds.forEachAlive(function(item:OptionsText) {
			var parent:OptionsText = grpOptions.members[item.ID];
	...
	function swapMode()
	{
		...
		bg.color = onKeyboardMode ? gamepadColor : keyboardColor;
		onKeyboardMode = !onKeyboardMode;
		
		curSelected = 0;
		curAlt = false;
		controllerSpr.loadGraphic(Paths.image('menu/options/' + (onKeyboardMode ? 'keyboard' : 'controller')));
		controllerSpr.setGraphicSize(0, 82);
		controllerSpr.updateHitbox();
		createTexts();
	}
