...
class FlxUIInputTextEx extends FlxUIInputText
{
	...
	override function getCaretIndex():Int
	{
		#if FLX_MOUSE
		var position = FlxG.mouse.getViewPosition(getDefaultCamera());
		var pos = getScreenPosition(getDefaultCamera());
		
		var hit = FlxPoint.get(position.x - pos.x, position.y - pos.y);
